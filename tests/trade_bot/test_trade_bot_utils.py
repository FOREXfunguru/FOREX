...
from trade_bot.trade_bot_utils import (
    adjust_SL_pips,
    get_trade_type,
    adjust_SL_candles,
    adjust_SL_nextSR,
...
datetimes = [
    (
        datetime.datetime(2018, 4, 27, 22, 0, 0),
        datetime.datetime(2020, 4, 27, 21, 0, 0),
        "short",
    ),
    (
        datetime.datetime(2018, 5, 18, 21, 0, 0),
        datetime.datetime(2020, 3, 18, 21, 0, 0),
        "long",
    ),
    (
        datetime.datetime(2018, 6, 17, 21, 0, 0),
        datetime.datetime(2020, 1, 17, 21, 0, 0),
        "long",
    ),
    (
        datetime.datetime(2018, 7, 11, 21, 0, 0),
        datetime.datetime(2019, 8, 11, 21, 0, 0),
        "long",
    ),
    (
        datetime.datetime(2018, 1, 9, 21, 0, 0),
        datetime.datetime(2019, 1, 9, 21, 0, 0),
        "short",
    ),
]


@pytest.mark.parametrize("start," "end," "type", datetimes)
def test_get_trade_type(start, end, type, clO_pickled):
    new_cl = clO_pickled.slice(start=start, end=end)
