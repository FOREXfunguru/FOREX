...
class Candle(object):
    ...
    ---------------
    dict_data : dict
                Dictionary with data as generated by
                the Oanda REST API
    """
    ...
    def volatile_c(self, diff_cutoff, bit, pair ):
        '''
        ...
        Return
        ------
        bool : True if is volatile.
               False otherwise
        diff_cutoff : int
                      Number of pips used as the cutt-off
                      above which the candle is considered volatile
                      It is basically the diff in pips between high
                      and Low of the candle
        bit : str
              Candle bit used. i.e. Ask or Bid
        pair : str
               Instrument
        '''
        u_attr = "high{0}".format(bit)
        l_attr = "low{0}".format(bit)
        diff_in_price = getattr(self, u_attr)-getattr(self, l_attr)
